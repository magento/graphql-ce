...
{
    /**
     * Verify that layered navigation filters are returned
     * for product query for products filtered by sku
     *
     * @magentoApiDataFixture Magento/Catalog/_files/products_with_layered_navigation_attribute2.php
     * @SuppressWarnings(PHPMD.ExcessiveMethodLength)
     */
    public function testLayeredNavigationFiltersForProductsFilteredBySku()
    {
        $query = <<<QUERY
{
    products (
...
QUERY;
        ...
        $this->assertFilters(
            $response,
            $this->getExpectedFiltersDataSetForProductsFilteredBySku(),
     ...
     */
    private function getExpectedFiltersDataSetForProductsFilteredBySku()
    {
        ...
        return [
            ...
            [
                'name' => 'Test Simple Nav Attr 2',
                'filter_items_count' => 2,
                'request_var' => 'test_simple_nav_attr_2',
                'filter_items' => [
                    [
                        'label' => 'Option 1',
                    ...
                    [
                        'label' => 'Option 2',
                        'value_string' => $options[2]->getValue(),
                        'items_count' => 2,
                    ],
                ],
            ],
    ...
    /**
     * Verify that layered navigation filters are returned
     * for product query for products filtered by price
     *
     * @magentoApiDataFixture Magento/Catalog/_files/products_with_layered_navigation_attribute2.php
     * @SuppressWarnings(PHPMD.ExcessiveMethodLength)
     */
    public function testLayeredNavigationFiltersForProductsFilteredByPrice()
    {
        $query = <<<QUERY
{
    products (
...
QUERY;
        ...
        $this->assertFilters(
            $response,
            $this->getExpectedFiltersDataSetForProductsFilteredByPrice(),
            'Returned filters data set does not match the expected value'
        );
    }
     ...
     * @throws \Magento\Framework\Exception\LocalizedException
     */
    private function getExpectedFiltersDataSetForProductsFilteredByPrice()
    {
        /** @var \Magento\Eav\Model\Config $eavConfig */
        $eavConfig = \Magento\TestFramework\Helper\Bootstrap::getObjectManager()->get(\Magento\Eav\Model\Config::class);
        $attribute = $eavConfig->getAttribute('catalog_product', 'test_simple_nav_attr_2');
    ...
    private function assertFilters($response, $expectedFilters, $message = '')
    {
        ...
        $this->assertEquals($expectedFilters, $response['products']['filters']);
    }
